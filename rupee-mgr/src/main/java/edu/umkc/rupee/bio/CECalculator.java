...
public class CECalculator {
	...
	public AFPChain extractFragments(AFPChain afpChain,
		...
		this.mat = initSumOfDistances(nse1, nse2, winSize, winSizeComb1, ca1, ca2);
	...
	private double[][] initIntraDistmatrix(Atom[] ca, int nse) throws StructureException
	{
		...
		for(int ise1=0; ise1<nse; ise1++)  {
			for(int ise2=0; ise2<nse; ise2++)  {
				intraDist[ise1][ise2] = Calc.getDistance(ca[ise1], ca[ise2]);
			}
		}
		return intraDist;
	}
	...
	public double[][] initSumOfDistances(int nse1, int nse2, int winSize, int  winSizeComb1, Atom[] ca1, Atom[] ca2) {
		...
		for(int ise1=0; ise1<nse1; ise1++) {
			for(int ise2=0; ise2<nse2; ise2++) {
				...
				// sum up the differences of intra residues distances
				d=0.0;
	...
	public void traceFragmentMatrix( AFPChain afpChain,
		...
		int iterDepth;
		if ( gapMax > 0){
            // multiplied by 2 for alternating the gaps
            ...
            for(int ise1_=ise11; ise1_<ise12; ise1_++) {
                ise2Loop:
                    for(int ise2_=ise21; ise2_<ise22; ise2_++) {
                        ...
                        if((ise1>nse1-winSize*(nBestTrace-1) || ise2>nse2-winSize*(nBestTrace-1))) continue ise2Loop;
                            ...
                            while(nTrace>0) {
                                ...
                                if(isTraceUp) {
                                    ...
                                    itLoop:
                                        ...
                                        // the iterDepth parameter accounts for much of algorithm performance
                                        // how can this be dynamically optimized without messing up results
                                        // or maybe it is the result that is carried away from loop
                                        // that effects performance elsewhere
                                        for(int it=0; it<iterDepth; it++) {

                                            // alternately check the gaps default 30 * 2 = 60 - 30 on each side at the most but only one-sided
                                            jgap=(it+1)/2;
                                            jdir=(it+1)%2;
                                            ...
                                            // only extend if differences of distances meets criteria 
                                            // the fragments have to be somewhat similar internally
                                            ...
                                            // this is checking that they match up to the internal structure
                                            // of existing fragments on the path
                                            ...
                                            // new candidate trace that meets criterion
                                            score2=score1;
	...
	protected double getScoreFromDistanceMatrices(int mse1, int mse2,int winSize) {
        ...
        // i think this is determining whether or not to add a new AFP to a trace
        // by comparing an AFP mse1 and mse2 outside the trace to ones
        // inside the trace and this will ultimately be averages in the calling
        // function
        //
        // for mse1 and mse2 it is going to walk the trace (or path) 
        // and sum up the differences of distances 
        // i think itrace is better path fragment and trace1 and trace2 are path1 path2 better names
        //
        // i wonder if this could be speed up by not walking back too far
        // if the distances bare a good relation to the first and the last is that enough
        //
        // it seems this would be better parameterized rather than sacrificing gap size

		double score = 0;
		for(int itrace=0; itrace<nTrace; itrace++) {
			score +=  Math.abs(dist1[trace1[itrace]][mse1] - dist2[trace2[itrace]][mse2]);
			score +=  Math.abs(dist1[trace1[itrace]+winSize-1][mse1+winSize-1] - dist2[trace2[itrace]+winSize-1][mse2+winSize-1]);
			for(int id=1; id<winSize-1; id++)
				score+=  Math.abs(dist1[trace1[itrace]+id][mse1+winSize-1-id] - dist2[trace2[itrace]+id][mse2+winSize-1-id]);
		}
