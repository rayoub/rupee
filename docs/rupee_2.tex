...
If the same process is performed on a pair of sets, the key result is that the probability of matching min-hashes is equal to the Jaccard similarity of the two sets \cite{Broder1998}.
...
In our case, the sets contain shingle hashes for a protein structure from which we obtain 99 min-hashes for each set as described in \cite{Rajaraman2012}. 
...
In fast mode, the top 8,000 matches from the min-hashing and LSH initial filtering are obtained.
For each match, we calculate the exact denominator in the definition of Jaccard similarity from the original shingle sequences of the structures.
